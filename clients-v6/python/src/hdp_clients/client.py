...
class HDPClient:
    ...
    def _headers(self, *, mutation: bool = False) -> dict[str, str]:
        ...
        if self.token:
            ...
            if mutation: headers["X-HDP-CSRF"] = "1"
        return headers

    def _request(self, method: str, path: str, *, params: Mapping[str, Any] | None = None, json: Any = None) -> Any:
        url = f"{self.base_url.rstrip('/')}/{path.lstrip('/')}"
        try:
            response = httpx.request(method, url, params=params, json=json, headers=self._headers(mutation=method.upper() not in {"GET","HEAD","OPTIONS"}), timeout=self.timeout, follow_redirects=False)
        except httpx.HTTPError as exc:
            raise HDPClientError(f"HDP inaccessible: {exc}") from exc
        if response.is_error:
            try:
                payload=response.json(); detail=payload.get("detail",payload) if isinstance(payload,dict) else payload
    ...
    def world_bank_descriptor(self): return self._request("GET","/api/providers/world-bank-health/descriptor")
    def world_bank_effective_configuration(self, *, project_id=None): return self._request("GET","/api/providers/world-bank-health/configuration/effective",params={"project_id":project_id} if project_id else None)
    def world_bank_observations(self, *, country:str, indicator:str, date:str="", source:int=2, page:int=1, per_page:int=50, mrv:int|None=None, mrnev:int|None=None, gapfill:bool=False, frequency:str="", footnote:bool=False, language:str="en", project_id:str|None=None):
        body={"country":country,"indicator":indicator,"date":date,"source":source,"page":page,"per_page":per_page,"mrv":mrv,"mrnev":mrnev,"gapfill":gapfill,"frequency":frequency,"footnote":footnote,"language":language}
        if project_id: body["project_id"]=project_id
        return self._request("POST","/api/providers/world-bank-health/observations",json=body)
    def world_bank_metadata(self, *, query:str, source:int=2, page:int=1, per_page:int=1000, language:str="en"):
        return self._request("POST","/api/providers/world-bank-health/metadata",json={"query":query,"source":source,"page":page,"per_page":per_page,"language":language})
    ...
    def search(self,*,project_id,source,query,result_limit=25,auto_download=False,parameters=None): return self._request("POST","/api/search",json={"project_id":project_id,"source":source,"query":query,"result_limit":result_limit,"auto_download":auto_download,"parameters":dict(parameters or {})})
    def federated_search(self,*,project_id,sources,query,result_limit=25,auto_download=False,parameters_by_source=None):
        results=[]
        for source in sources:
            try: results.append({"source":source,"ok":True,"result":self.search(project_id=project_id,source=source,query=query,result_limit=result_limit,auto_download=auto_download,parameters=(parameters_by_source or {}).get(source,{}))})
            except HDPClientError as exc: results.append({"source":source,"ok":False,"error":str(exc)})
        return results
