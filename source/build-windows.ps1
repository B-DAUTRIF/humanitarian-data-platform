...
$installerName = "HumanitarianDataPlatform_Setup_Native_GUI_v6.0.0.exe"
...
try {
    ...
    & $buildScript
    if ($LASTEXITCODE -ne 0) { throw "La compilation MSVC a échoué ($LASTEXITCODE)." }
}
finally { Pop-Location }

if (-not (Test-Path -LiteralPath $installerPath -PathType Leaf)) { throw "L'installateur attendu n'a pas été produit : $installerPath" }
$version = (Get-Item -LiteralPath $installerPath).VersionInfo
if ($version.FileVersion -notlike "6.0.0*" -or $version.ProductVersion -notlike "6.0.0*") { throw "Métadonnées inattendues : FileVersion=$($version.FileVersion), ProductVersion=$($version.ProductVersion)" }
$toolsDirectory = Join-Path $visualStudio "VC\Tools\MSVC"
$dumpbinExecutable = Get-ChildItem -LiteralPath $toolsDirectory -Filter dumpbin.exe -Recurse | Where-Object { $_.FullName -match 'Hostx64\\x64\\dumpbin\.exe$' } | Select-Object -First 1
if (-not $dumpbinExecutable) { throw "dumpbin.exe x64 introuvable." }
$headers = & $dumpbinExecutable.FullName /headers $installerPath | Out-String
if ($LASTEXITCODE -ne 0 -or $headers -notmatch "machine \(x64\)" -or $headers -notmatch "Windows GUI") { throw "Le contrôle PE32+ GUI x64 a échoué." }
