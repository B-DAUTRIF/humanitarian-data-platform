...
title HDP 3.0.0 - Configuration GitHub

echo.
echo ============================================================
echo   Humanitarian Data Platform 3.0.0 - Configuration GitHub
echo ============================================================
echo.
echo Ce correctif enregistre le jeton dans le fichier .env local,
echo recree uniquement les conteneurs API et passerelle GitHub,
echo puis verifie la configuration.
...
if errorlevel 1 (
  call :fail "La configuration locale a echoue. Les conteneurs n'ont pas ete recrees."
  exit /b 1
)

echo.
echo Recreation de l'API et de la passerelle GitHub...
pushd "%HDP_APP%"
docker compose up -d --no-deps --force-recreate github-api api
if errorlevel 1 (
  popd
  call :fail "Docker n'a pas pu recreer les services. Le jeton reste enregistre dans .env."
  exit /b 1
)
...
set "HDP_GATEWAY_TOKEN_STATUS="
for /f "usebackq delims=" %%S in (`docker compose exec -T github-api python -c "import os; print('CONFIGURE' if os.getenv('GITHUB_TOKEN', '').strip() else 'ABSENT')" 2^>nul`) do set "HDP_GATEWAY_TOKEN_STATUS=%%S"
popd
...
if /i not "%HDP_GATEWAY_TOKEN_STATUS%"=="CONFIGURE" (
  call :fail "La passerelle ne voit pas GITHUB_TOKEN. Consultez docker compose logs github-api."
  exit /b 1
)
...
echo.
echo ============================================================
echo   SUCCES : GITHUB_TOKEN est configure dans les deux services.
echo ============================================================
echo.
