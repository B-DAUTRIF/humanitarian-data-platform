...
class WorldBankObservationRequest(BaseModel):
    ...
    date: str = Field(default="", max_length=9)
...
async def observations(payload: WorldBankObservationRequest) -> dict[str, Any]:
    try:
        ...
        return {
            "provider": "world-bank-health",
            "status": "success" if normalized else "empty_valid",
...
async def metadata(payload: WorldBankMetadataRequest) -> dict[str, Any]:
    try:
        ...
        return {"provider": "world-bank-health", "status": "success" if normalized else "empty_valid", "count": len(normalized), "items": normalized, "native_response": raw, "native_request": native_request}
    except Exception as exc:
        raise _http_error(exc) from exc
...
async def indicators(source: int = Query(2, ge=1), page: int = Query(1, ge=1), per_page: int = Query(1000, ge=1, le=50000), language: str = Query("en", pattern=r"^(en|fr|es|ar|zh)$")) -> dict[str, Any]:
    try:
        runtime, _, _ = _contexts(None); service = WorldBankHealthService(runtime)
        raw, rows, native = await service.list_indicators(source=source, page=page, per_page=per_page, language=language)
        return {"provider":"world-bank-health", "count":len(rows), "items":rows, "native_response":raw, "native_request":native}
...
async def countries(identifier: str = "", page: int = Query(1, ge=1), per_page: int = Query(1000, ge=1, le=50000), language: str = Query("en", pattern=r"^(en|fr|es|ar|zh)$")) -> dict[str, Any]:
    try:
        runtime, _, _ = _contexts(None); service = WorldBankHealthService(runtime)
        raw, rows, native = await service.list_countries(identifier=identifier, page=page, per_page=per_page, language=language)
        return {"provider":"world-bank-health", "count":len(rows), "items":rows, "native_response":raw, "native_request":native}
    except Exception as exc: raise _http_error(exc) from exc
...
async def topics(identifier: str = "", page: int = Query(1, ge=1), per_page: int = Query(1000, ge=1, le=50000), language: str = Query("en", pattern=r"^(en|fr|es|ar|zh)$")) -> dict[str, Any]:
    try:
        runtime, _, _ = _contexts(None); service = WorldBankHealthService(runtime)
        raw, rows, native = await service.list_topics(identifier=identifier, page=page, per_page=per_page, language=language)
        return {"provider":"world-bank-health", "count":len(rows), "items":rows, "native_response":raw, "native_request":native}
    except Exception as exc: raise _http_error(exc) from exc
...
async def sources(identifier: str = "", page: int = Query(1, ge=1), per_page: int = Query(1000, ge=1, le=50000), language: str = Query("en", pattern=r"^(en|fr|es|ar|zh)$")) -> dict[str, Any]:
    try:
        runtime, _, _ = _contexts(None); service = WorldBankHealthService(runtime)
        raw, rows, native = await service.list_sources(identifier=identifier, page=page, per_page=per_page, language=language)
        return {"provider":"world-bank-health", "count":len(rows), "items":rows, "native_response":raw, "native_request":native}
    except Exception as exc: raise _http_error(exc) from exc
...
async def indicator_metadata(indicator: str, source: int = Query(2, ge=1), language: str = Query("en", pattern=r"^(en|fr|es|ar|zh)$")) -> dict[str, Any]:
    try:
        ...
        return {"provider":"world-bank-health", "status":"success" if rows else "empty_valid", "count":len(rows), "items":rows, "native_response":raw, "native_request":native}
    except Exception as exc: raise _http_error(exc) from exc
...
def world_bank_ui() -> str:
    return """<!doctype html><html lang='fr'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>HDP — World Bank Health</title><style>body{font-family:Segoe UI,Arial,sans-serif;background:#f5f7fa;color:#172033;margin:0}header{padding:18px 24px;background:#172033;color:white}main{max-width:1500px;margin:auto;padding:20px}.tabs,.actions{display:flex;gap:8px;flex-wrap:wrap;margin:10px 0}.panel{background:white;border:1px solid #d9dee7;border-radius:8px;padding:14px;margin:10px 0}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:10px}input,select,button{font:inherit;padding:8px;border:1px solid #b8c0cc;border-radius:6px;box-sizing:border-box;width:100%}button{width:auto;cursor:pointer}pre{white-space:pre-wrap;overflow:auto;max-height:650px;background:#111827;color:#e5e7eb;padding:14px;border-radius:8px}.hidden{display:none}label{display:flex;flex-direction:column;gap:4px;font-size:13px}.hint{color:#586174;font-size:13px}</style></head><body><header><h1>World Bank Health / WDI dans HDP</h1><div>Tous les paramètres World Bank qualifiés sont exposés. Les agrégats sont distingués des pays via le catalogue fournisseur versionné.</div></header><main><div class='tabs'><button data-mode='simple'>Simple</button><button data-mode='advanced'>Avancé</button><button data-mode='expert'>Expert</button><button onclick="location.href='/'">Retour HDP</button></div><div class='panel'><div class='grid'><label>Projet (optionnel)<input id='project'></label><label>Recherche catalogue<input id='query' placeholder='malaria'></label><label>Pays ISO3<input id='country' value='RWA'></label><label>Indicateur(s)<input id='indicator' placeholder='SH.MLR.INCD.P3'></label><label>Année / intervalle<input id='date' placeholder='2020:2025'></label><label>Limite HDP<input id='result_limit' type='number' min='1' max='100' value='25'></label></div></div><div id='advanced' class='panel hidden'><h3>Paramètres natifs qualifiés</h3><div class='grid'><label>source<input id='source' type='number' value='2'></label><label>page<input id='page' type='number' value='1'></label><label>per_page<input id='per_page' type='number' value='50'></label><label>mrv<input id='mrv' type='number'></label><label>mrnev<input id='mrnev' type='number'></label><label>gapfill<select id='gapfill'><option value='false'>false</option><option value='true'>true</option></select></label><label>frequency<select id='frequency'><option value=''></option><option>Y</option><option>Q</option><option>M</option></select></label><label>footnote<select id='footnote'><option value='false'>false</option><option value='true'>true</option></select></label><label>format<select id='format'><option>json</option></select></label><label>language<select id='language'><option>en</option><option>fr</option><option>es</option><option>ar</option><option>zh</option></select></label></div></div><div id='expert' class='panel hidden'><h3>Catalogue et provenance</h3><div class='actions'><button id='descriptor'>Descripteur</button><button id='config'>Configuration</button><button id='countries'>Catalogue géographique versionné</button><button id='metadata'>Métadonnées</button></div><pre id='expertout'>Prêt.</pre></div><div class='panel'><div class='actions'><button id='run'>Interroger</button></div><pre id='result'>Aucune requête.</pre></div><script>const q=s=>document.querySelector(s);function cookie(n){return document.cookie.split(';').map(x=>x.trim()).find(x=>x.startsWith(n+'='))?.slice(n.length+1)||''}async function req(path,method='GET',body=null){const o={method,credentials:'same-origin',headers:{Accept:'application/json'}};if(body){o.headers['Content-Type']='application/json';o.headers['x-hdp-csrf']=decodeURIComponent(cookie('hdp_csrf'));o.body=JSON.stringify(body)}const r=await fetch(path,o),x=await r.json();if(!r.ok)throw new Error(JSON.stringify(x.detail||x));return x}function body(){const v={project_id:q('#project').value||null,country:q('#country').value,indicator:q('#indicator').value,date:q('#date').value,source:Number(q('#source').value),page:Number(q('#page').value),per_page:Number(q('#per_page').value),mrv:q('#mrv').value?Number(q('#mrv').value):null,mrnev:q('#mrnev').value?Number(q('#mrnev').value):null,gapfill:q('#gapfill').value==='true',frequency:q('#frequency').value,footnote:q('#footnote').value==='true',language:q('#language').value};return v}q('#run').onclick=async()=>{try{const x=await req('/api/providers/world-bank-health/observations','POST',body());q('#result').textContent=JSON.stringify(x.items,null,2);q('#expertout').textContent=JSON.stringify(x.native_request,null,2)}catch(e){q('#result').textContent='ERREUR '+e.message}};q('#descriptor').onclick=async()=>q('#expertout').textContent=JSON.stringify(await req('/api/providers/world-bank-health/descriptor'),null,2);q('#config').onclick=async()=>{const id=q('#project').value;q('#expertout').textContent=JSON.stringify(await req('/api/providers/world-bank-health/configuration/effective'+(id?'?project_id='+encodeURIComponent(id):'')),null,2)};q('#countries').onclick=async()=>q('#expertout').textContent=JSON.stringify(await req('/api/providers/world-bank-health/geography-vocabulary'),null,2);q('#metadata').onclick=async()=>{try{q('#expertout').textContent=JSON.stringify(await req('/api/providers/world-bank-health/metadata','POST',{query:q('#query').value||q('#indicator').value,source:Number(q('#source').value),language:q('#language').value}),null,2)}catch(e){q('#expertout').textContent='ERREUR '+e.message}};document.querySelectorAll('[data-mode]').forEach(b=>b.onclick=()=>{const m=b.dataset.mode;q('#advanced').classList.toggle('hidden',m==='simple');q('#expert').classList.toggle('hidden',m!=='expert')});</script></main></body></html>"""
