...
def sources():
    ...
    for slug in sorted({str(r["source_slug"]) for r in rows}):
        rs = source_rows(slug)
        origins = sorted({str(r.get("origin") or "inconnu") for r in rs})
        docs = sorted({str(r.get("documentation_url") or "") for r in rs if r.get("documentation_url")})
        items.append({
            "slug": slug,
            "name": rs[0]["Source"],
            "parameters": len(rs),
            ...
            "machine_verified": any("OpenAPI" in o or "Swagger" in o for o in origins),
        })
    return {
        "parameters": len(rows),
        "sources": len(items),
