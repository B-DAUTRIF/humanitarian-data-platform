...
import unittest
from source.payload.api.app.reliefweb_v2 import DEFAULT_APPNAME, ReliefWebValidationError, build_payload, request_spec, resolve_appname, validate_filter

class ReliefWebV2Tests(unittest.TestCase):
    def test_default_appname(self):
        x=resolve_appname({}, {})
        self.assertEqual(x.value, DEFAULT_APPNAME); self.assertEqual(x.origin, 'default')
    ...
    def test_recursive_filter(self):
        f=validate_filter({'operator':'AND','conditions':[{'field':'country','value':'Rwanda'},{'operator':'OR','conditions':[{'field':'format','value':'Map'},{'field':'format','value':'Infographic'}]}]})
        self.assertEqual(f['conditions'][1]['operator'],'OR')
    ...
    def test_complex_request_uses_post(self):
        r=request_spec('reports',{'filter':{'operator':'AND','conditions':[{'field':'country','value':'Rwanda'},{'field':'theme','value':'Health'}]}},global_settings={})
        self.assertEqual(r['method'],'POST'); self.assertEqual(r['appname'],DEFAULT_APPNAME)
    def test_simple_request_uses_get(self):
        r=request_spec('reports',{'query':'malaria','limit':10},global_settings={}); self.assertEqual(r['method'],'GET')
    def test_item_only_profile_fields(self):
        r=request_spec('reports',{'query':'ignored','profile':'full','fields_include':['source.name']},item_id=1082221)
        self.assertNotIn('query',r['payload']); self.assertEqual(r['method'],'GET')
    def test_nine_content_types(self):
        for content_type in ('reports','disasters','countries','jobs','training','sources','blog','book','references'):
            self.assertIn('/'+content_type, request_spec(content_type,{'limit':1})['url'])
