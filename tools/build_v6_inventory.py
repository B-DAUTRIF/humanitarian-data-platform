...
SPEC_CANDIDATES: dict[str, list[str]] = {
    "un-sdg": ["https://unstats.un.org/SDGAPI/swagger/v1/swagger.json", "https://unstats.un.org/sdgapi/swagger/v1/swagger.json"],
    "hdx-hapi": ["https://hapi.humdata.org/openapi.json", "https://hapi.humdata.org/api/openapi.json"],
    "gdacs": ["https://www.gdacs.org/gdacsapi/swagger/v1/swagger.json", "https://www.gdacs.org/gdacsapi/swagger/swagger.json"],
    "unhcr": ["https://api.unhcr.org/openapi.json", "https://api.unhcr.org/swagger.json"],
    "dhs": ["https://api.dhsprogram.com/swagger/docs/v1", "https://api.dhsprogram.com/swagger/v1/swagger.json"],
}


def ui_control(schema: dict[str, Any], readonly: bool = False, sensitive: bool = False) -> str:
    if sensitive: return "secret / variable d’environnement"
    if readonly: return "information en lecture seule"
    typ = schema.get("type")
    if schema.get("enum"): return "liste de sélection"
    if typ == "boolean": return "case à cocher"
    if typ in {"integer", "number"}: return "champ numérique"
    if typ == "array": return "liste / sélection multiple"
    if typ == "object": return "éditeur JSON structuré"
...
def base_row(source_id: str, *, operation: str, method: str, endpoint: str, name: str,
    ...
    return {
        "Source": SOURCE_NAMES.get(source_id, source_id), "source_slug": source_id,
        "Opération": operation, "Méthode": method, "Endpoint": endpoint, "Paramètre": name,
        "Emplacement": location, "Type": schema.get("type", "string"), "Obligatoire": bool(required),
        ...
        "readonly": bool(readonly), "supported": bool(supported), "sensitive": bool(sensitive),
        "origin": origin, "documentation_url": documentation_url, "default": schema.get("default"),
        "enum": schema.get("enum"), "minimum": schema.get("minimum"), "maximum": schema.get("maximum"),
...
def registry_rows() -> list[dict[str, Any]]:
    ...
    for source_id in CONNECTORS:
        d = connector_definition(source_id); doc = (d.get("documentation_evidence") or [""])[0]
        for scope, schema, operation, method, location in [
            ...
            for name, spec in schema.get("properties", {}).items():
                readonly = bool(spec.get("readOnly")); sensitive = any(x in name.casefold() for x in ("secret", "token", "password"))
                rows.append(base_row(source_id, operation=operation, method=method, endpoint=d["base_url"], name=name,
                    location=location, schema=spec, required=name in required, supported=not readonly, readonly=readonly,
                    sensitive=sensitive, origin=f"source_registry:{scope}", documentation_url=doc))
        for item in CURATED_NATIVE.get(source_id, []):
            rows.append(base_row(source_id, operation=item["operation"], method=item["method"], endpoint=item["endpoint"],
                ...
                sensitive=bool(item.get("sensitive", False)), origin="provider documentation / curated V6 baseline", documentation_url=doc))
    return rows
...
def fetch_json(url: str) -> dict[str, Any] | None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=20) as response:
            ...
            data = json.load(response); return data if isinstance(data, dict) else None
    except (urllib.error.URLError, urllib.error.HTTPError, TimeoutError, json.JSONDecodeError, OSError): return None


def resolve_ref(spec: dict[str, Any], value: Any) -> Any:
    if not isinstance(value, dict) or "$ref" not in value: return value
    ref = value["$ref"]
    if not isinstance(ref, str) or not ref.startswith("#/"): return value
    node: Any = spec
...
def schema_from_parameter(spec: dict[str, Any], parameter: dict[str, Any]) -> dict[str, Any]:
    parameter = resolve_ref(spec, parameter); schema = resolve_ref(spec, parameter.get("schema", {})) if isinstance(parameter, dict) else {}
    if not isinstance(schema, dict): schema = {}
    if "type" not in schema and isinstance(parameter, dict) and parameter.get("type"): schema["type"] = parameter.get("type")
    if isinstance(parameter, dict) and parameter.get("enum") and "enum" not in schema: schema["enum"] = parameter.get("enum")
    return schema or {"type": "string"}


def openapi_rows(source_id: str, spec: dict[str, Any], spec_url: str) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []; paths = spec.get("paths", {})
    if not isinstance(paths, dict): return rows
    for path, path_item in paths.items():
        if not isinstance(path_item, dict): continue
        path_parameters = path_item.get("parameters")
        common = path_parameters if isinstance(path_parameters, list) else []
        for method, operation in path_item.items():
            if method.lower() not in {"get", "post", "put", "patch", "delete", "head", "options"} or not isinstance(operation, dict): continue
            ...
            if isinstance(op_parameters, list): parameters += op_parameters
            for raw in parameters:
                p = resolve_ref(spec, raw)
                if not isinstance(p, dict) or not p.get("name"): continue
                schema = schema_from_parameter(spec, p); name = str(p["name"])
                sensitive = name.casefold() in {"authorization", "api_key", "apikey", "token", "password"}
                rows.append(base_row(source_id, operation=str(op_name), method=method.upper(), endpoint=str(path), name=name,
                    location=str(p.get("in", "query")), schema=schema, required=bool(p.get("required", False)),
                    description=str(p.get("description", "")), supported=False, readonly=True, sensitive=sensitive,
                ...
                if isinstance(content, dict):
                    for media in content.values():
                        ...
                        if isinstance(body_schema, dict):
                            props = body_schema.get("properties", {}); required_body = set(body_schema.get("required", []))
                            if isinstance(props, dict):
                                for name, raw_schema in props.items():
                                    body_prop = resolve_ref(spec, raw_schema)
                                    if not isinstance(body_prop, dict): body_prop = {"type": "string"}
                                    rows.append(base_row(source_id, operation=str(op_name), method=method.upper(), endpoint=str(path),
                                        name=str(name), location="body", schema=body_prop, required=name in required_body,
                                        description=str(body_prop.get("description", "")), supported=False, readonly=True,
                                        origin="provider OpenAPI requestBody", documentation_url=spec_url))
    return rows


def live_spec_rows() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    rows: list[dict[str, Any]] = []; report: dict[str, Any] = {}; offline = os.getenv("HDP_INVENTORY_OFFLINE", "0") == "1"
    for source_id, candidates in SPEC_CANDIDATES.items():
        report[source_id] = {"status": "not_found", "url": None, "rows": 0}
        if offline: report[source_id]["status"] = "offline"; continue
        for url in candidates:
            spec = fetch_json(url)
            if not spec: continue
            extracted = openapi_rows(source_id, spec, url)
            if extracted:
                rows.extend(extracted); report[source_id] = {"status": "loaded", "url": url, "rows": len(extracted)}; break
    return rows, report
...
def deduplicate(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        key=(row["source_slug"],row["Opération"],row["Méthode"],row["Endpoint"],row["Emplacement"],row["Paramètre"]); current=chosen.get(key)
        if current is None or (row.get("supported") and not current.get("supported")): chosen[key]=row
...
def write_parts(rows: list[dict[str, Any]], part_size: int = 400) -> None:
    ...
    for old in OUT_DIR.glob("part*.jsonl"): old.unlink()
    for i in range(0,len(rows),part_size):
        path=OUT_DIR/f"part{i//part_size:03d}.jsonl"
        with path.open("w",encoding="utf-8",newline="\n") as handle:
            for row in rows[i:i+part_size]: handle.write(json.dumps(row,ensure_ascii=False,sort_keys=True)+"\n")


def write_docs(rows: list[dict[str, Any]], spec_report: dict[str, Any]) -> None:
    DOC_DIR.mkdir(parents=True,exist_ok=True); by_source: dict[str,list[dict[str,Any]]]=defaultdict(list)
    for row in rows: by_source[row["source_slug"]].append(row)
    with (DOC_DIR/"API_INVENTORY.csv").open("w",encoding="utf-8-sig",newline="") as handle:
        fields=list(rows[0].keys()) if rows else []; writer=csv.DictWriter(handle,fieldnames=fields); writer.writeheader(); writer.writerows(rows)
    md=["# HDP V6.0.0 — Inventaire des paramètres API","",
        ...
        f"**Total : {len(rows)} entrées · {len(by_source)} sources.**","","## État des spécifications machine","","| Source | État | URL | Paramètres extraits |","|---|---|---|---:|"]
    for source_id in CONNECTORS:
        info=spec_report.get(source_id,{"status":"n/a","url":None,"rows":0}); md.append(f"| {SOURCE_NAMES.get(source_id,source_id)} | {info.get('status','n/a')} | {info.get('url') or ''} | {info.get('rows',0)} |")
    for source_id in CONNECTORS:
        items=by_source[source_id]; md += ["",f"## {SOURCE_NAMES.get(source_id,source_id)}","",f"{len(items)} entrées cataloguées, dont {sum(bool(r.get('supported')) for r in items)} directement prises en charge par HDP.","","| Opération | Méthode | Endpoint | Paramètre | Emplacement | Type | UI | Pris en charge | Origine |","|---|---|---|---|---|---|---|---|---|"]
        for r in items:
...
def validate(rows: list[dict[str, Any]]) -> None:
    sources={r["source_slug"] for r in rows}; missing=set(CONNECTORS)-sources
    if missing: raise SystemExit(f"Sources absentes de l'inventaire: {sorted(missing)}")
    keys={(r["source_slug"],r["Opération"],r["Méthode"],r["Endpoint"],r["Emplacement"],r["Paramètre"]) for r in rows}
    if len(keys)!=len(rows): raise SystemExit("Doublons non résolus dans l'inventaire")
    for source_id in CONNECTORS:
        d=connector_definition(source_id)
        for scope_name,schema in (("global",d["global_settings_schema"]),("project",d["project_schema"])):
            for name in schema.get("properties",{}):
                if not any(r["source_slug"]==source_id and r["Paramètre"]==name for r in rows): raise SystemExit(f"{source_id}:{scope_name}:{name} absent de l'inventaire")


def main() -> None:
    rows=registry_rows(); live_rows,spec_report=live_spec_rows(); rows=deduplicate(rows+live_rows); validate(rows); write_parts(rows); write_docs(rows,spec_report)
    print(json.dumps({"version":VERSION,"sources":len({r['source_slug'] for r in rows}),"entries":len(rows),"operations":len({(r['source_slug'],r['Opération'],r['Méthode'],r['Endpoint']) for r in rows}),"supported":sum(bool(r.get('supported')) for r in rows),"informational":sum(not bool(r.get('supported')) for r in rows),"spec_report":spec_report},ensure_ascii=False,sort_keys=True))
