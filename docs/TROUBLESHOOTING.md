...
1. Double-cliquez sur `HDP_Diagnostic_v2.3.2.cmd`.
2. Attendez la fin ; chaque commande externe est limitée à 15 secondes.
3. Récupérez `HDP_Debug_v2.3.2_*.log` sur le Bureau.
...
| Création GitHub indisponible | `GITHUB_TOKEN` absent ou conteneur API créé avant la modification de `.env` | Double-cliquer sur [`HDP_Configurer_GitHub_v2.3.2.cmd`](../source/HDP_Configurer_GitHub_v2.3.2.cmd), saisir le jeton dans l'invite masquée et attendre la vérification |
...
| Nouveau périmètre avec l'erreur de l'ancien pays | installation 2.3.1 conservant le dernier statut | Mettre à niveau vers 2.3.2, enregistrer le profil puis synchroniser ; le statut devient d'abord « synchronisation requise » |
| Soudan ou Algérie indiqué sans COD-AB | filtre 2.3.1 exigeant `dataseries_name`, absent du JSON CKAN actuel | Mettre à niveau vers 2.3.2 et relancer la synchronisation |
...
Le fichier [`HDP_Configurer_GitHub_v2.3.2.cmd`](../source/HDP_Configurer_GitHub_v2.3.2.cmd) applique la procédure complète sous Windows :
