# Prompt autonome de reconstruction — Humanitarian Data Platform 2.3.1
...
Tu reprends la maintenance et la livraison de **Humanitarian Data Platform (HDP) 2.3.1**, application locale Windows 10/11 x64. Ton objectif est de retrouver fidèlement l’état fonctionnel, documentaire et distribuable décrit ci-dessous, puis de poursuivre le projet sans régression et sans inventer de validation.
...
message `feat: restrict geodata to UN M49 official COD-AB v2.3.1`. Si `main` a
...
3. lire `dist/v2.3.1/MANIFESTE_HumanitarianDataPlatform_v2.3.1.txt` et `dist/v2.3.1/SHA256SUMS.txt` ;
...
Une exécution v1.5 a été validée sur Windows 11 avec Docker Desktop, WSL 2 et le port 18080. Cette recette historique ne doit pas être présentée comme une recette automatique de la v2.3.1.
...
La v2.3.1 remplace le modèle géographique 2.3.0 :
...
- diagnostic produit : `Bureau\HDP_Debug_v2.3.1_*.log`.
...
- profil géographique officiel avec `package_search`, filtre de série et validation locale ;
...
2. appeler `package_search` avec la série exacte
   `COD - Subnational Administrative Boundaries` ;
3. rejeter localement tout jeu dont la série, le niveau COD ou le groupe ISO3
...
source/
  build.sh
  HDP_Diagnostic_v2.3.1.cmd
  HDP_Configurer_GitHub_v2.3.1.cmd
  HumanitarianDataPlatform_Setup_README_v2.3.1.txt
  payload/
    CHANGELOG_HDP.log
...
tools/generate_notice_v231.py
```

### 20. Livrables v2.3.1 et empreintes
...
- `HumanitarianDataPlatform_Setup_Native_GUI_v2.3.1.exe.sha256` ;
- `HumanitarianDataPlatform_Windows_v2.3.1.zip` ;
- `HumanitarianDataPlatform_Source_v2.3.1.zip` ;
- `HumanitarianDataPlatform_Archive_complete_v2.3.1.zip` ;
- `HumanitarianDataPlatform_Archive_complete_v2.3.1.zip.sha256` ;
- `HumanitarianDataPlatform_Setup_README_v2.3.1.txt` ;
- `CHANGELOG_HDP_v2.3.1.log` ;
- `HDP_Configurer_GitHub_v2.3.1.cmd` ;
- `HDP_Diagnostic_v2.3.1.cmd` ;
- `Notice_detaillee_Humanitarian_Data_Platform_v2.3.1.pdf` ;
- `HDP_Prompt_exhaustif_reprise_GPT_Plus_v2.3.1.txt` ;
- `MANIFESTE_HumanitarianDataPlatform_v2.3.1.txt` ;
- `SHA256SUMS.txt`.
...
```text
EXE
a386c8868c5c258428c1e1df947f4aeb609c21b69abc7125e614509d52bd4c88

ZIP Windows
a2f187365c0c13bba3906cefb885813d538bc247b68b3d7bebe0a502f6432e53

ZIP sources
058c527d3fd19db3471b1f7d57d7931f463e3918dd6f24e3b6b65725b7dc79bf

Archive complète
Voir HumanitarianDataPlatform_Archive_complete_v2.3.1.zip.sha256 : son empreinte
ne peut pas être auto-incluse dans le prompt que contient cette même archive.

Notice PDF
fe3c3409ce52a11ec3d520ad8b751f15fa78cf0dcf350e608582d20d5cabf688
```
...
### 21. Validations réellement effectuées pour la v2.3.1
...
- contrôle des livrables historiques v2.3.0 et v2.0 ;
- PDF de 22 pages rendu en PNG et inspecté visuellement ;
- arbre GitHub distant relu après publication.

Le moteur Docker était absent de l’environnement Linux de construction. Par conséquent, ne prétends pas que les éléments suivants ont été validés pour la v2.3.1 :
...
Ce prompt décrit l’état de livraison v2.3.1 sans modifier les archives déjà hachées dans `dist/v2.3.1`.
