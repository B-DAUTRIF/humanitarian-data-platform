# Prompt autonome de reconstruction — Humanitarian Data Platform 2.5.0
...
Tu reprends la maintenance et la livraison de **Humanitarian Data Platform (HDP) 2.5.0**, application locale Windows 10/11 x64. Ton objectif est de retrouver fidèlement l’état fonctionnel, documentaire et distribuable décrit ci-dessous, puis de poursuivre le projet sans régression et sans inventer de validation.
...
L'autorité de la v2.5.0 est l'archive complète qui contient ce document, son
`MANIFESTE_HumanitarianDataPlatform_v2.5.0.txt` et son `SHA256SUMS.txt`. Au moment
de cette remise, le dépôt distant `main` est encore la v2.4.0 au commit
`d933af95fd93aa77e8bcaa621bdae9255f1d69c4`. Ne prétends pas que la v2.5.0 est
publiée tant qu'un nouveau commit distant n'a pas été envoyé puis relu.

Commence obligatoirement par :

1. vérifier que tu consultes le bon dépôt et la bonne branche ;
2. si l'archive v2.5.0 est fournie, vérifier son empreinte puis lire `README.md`
   et tous les fichiers de `docs/` ;
3. lire `dist/v2.5.0/MANIFESTE_HumanitarianDataPlatform_v2.5.0.txt` et `dist/v2.5.0/SHA256SUMS.txt` ;
...
- l’acquisition de métadonnées publiques ReliefWeb, HDX/CKAN, OMS/GHO,
  Banque mondiale/WDI, UNICEF/SDMX, ONU/ODD et DHS ;
- un catalogue de 18 sources mondiales séparant 7 API actives de 11 portails de
  référence ;
...
- un profil géographique listant COD‑AB et COD‑PS officiels et limité aux pays
  ou zones communs à ONU M49 et aux groupes HDX canoniques sélectionnés ;
- les sources, l’EXE, trois ZIP, la documentation, un diagnostic, un manifeste,
  un prompt de reprise et toutes les empreintes.
...
Une exécution v1.5 a été validée sur Windows 11 avec Docker Desktop, WSL 2 et le port 18080. Cette recette historique ne doit pas être présentée comme une recette automatique de la v2.5.0.
...
La v2.3.2 a corrigé deux régressions observées en exploitation :
...
La v2.4.0 ajoute la sélection explicite des familles COD officielles :

- COD-AB et COD-PS sont sélectionnables sous forme de liste ;
- COD-CS est visible mais désactivé tant que le registre vérifié embarqué reste vide ;
- COD-HP est visible comme famille retirée par OCHA et ne peut pas être sélectionné ;
- la zone est un pays ou une zone dans l'intersection ONU M49 × groupes HDX
  canoniques de toutes les familles choisies ;
- COD-AB utilise le format géospatial choisi ; COD-PS utilise CSV/XLSX ;
- une combinaison est atomique : si une famille manque, aucun sous-ensemble
  n'est téléchargé, mais les réponses CKAN et la décision sont archivées ;
- `cod_families` est ajouté au profil et `cod_family` à la provenance locale ;
- les anciens profils pays sont conservés avec COD-AB ; monde/région sont
  suspendus jusqu'à un choix explicite dans la nouvelle liste ;
- recette HDX du 7 août 2026 : 163 pays/zones COD-AB, 146 COD-PS, 143 communs ;
  Soudan est commun, Algérie est disponible uniquement pour COD-AB.

#### Version 2.5.0

La v2.5.0 ajoute les sources sanitaires mondiales :

- registre embarqué de 18 sources avec organisme, périmètre, domaines, accès,
  inscription éventuelle et liens officiels ;
- connecteurs interrogeables et planifiables : HDX, ReliefWeb, WHO Global Health
  Observatory, World Bank Health Indicators, UNICEF SDMX, UN SDG et DHS ;
- cinq nouveaux parseurs normalisent les indicateurs/flux mais la réponse
  distante originale reste archivée avec UUID, date UTC et SHA-256 ;
- WHO Mortality, WHO GLASS, WHO FluNet/FluID, WHO GHE, UNAIDS, IHME GHDx,
  UNICEF MICS, UN WPP, Global.health, WorldPop et Our World in Data restent
  explicitement des portails de référence ;
- aucune API publique stable n'est inventée pour une source qui ne la documente
  pas ; l'accès aux microdonnées DHS/MICS demeure une procédure distincte.
...
- diagnostic produit : `Bureau\HDP_Debug_v2.5.0_*.log`.
...
### 8. Acquisition et catalogue de sources
...
### 10. Profil géographique ONU M49 et familles COD officielles
...
- `m49_scope_code=null` avant choix explicite d'un nouveau profil ;
- `cod_families=["cod-ab"]` ;
- `official_policy=enhanced_preferred` ;
- `migration_required=true` tant qu'aucun pays/zone admissible n'est choisi ;
...
- `GET /api/cod/availability?families=cod-ab&families=cod-ps` ;
...
1. valider au moins une famille parmi `cod-ab` et `cod-ps` ; refuser COD-CS vide et COD-HP retiré ;
2. valider que `m49_scope_code` est un pays ou une zone M49 avec ISO3 ;
3. appeler `package_search` pour `name:cod-ab-*` et/ou `name:cod-ps-*` ;
4. exiger localement la série exacte ou le nom canonique exact `<famille>-<iso3>`,
   avec un unique groupe ISO3 présent dans M49 ;
5. pour COD-AB, exiger `cod-enhanced`/`cod-standard` et appliquer la politique ;
6. calculer la liste géographique par intersection des ISO3 valides ;
7. si une famille manque au moment du passage, ne télécharger aucun sous-ensemble ;
8. archiver toutes les réponses CKAN, la source M49, les familles, la décision,
   les jeux candidats, les absences et les formats manquants ;
9. sélectionner le format géospatial demandé pour COD-AB et CSV/XLSX pour COD-PS ;
10. associer famille, M49, ISO3, niveau publié, éditeur, licence et date à la ressource ;
11. réutiliser le pipeline commun de téléchargement sécurisé ;
12. produire `completed`, `partial`, `failed`, `no_official_dataset` ou
    `no_matching_resource`, puis persister l’état ;
13. en automatique, avancer la prochaine échéance avant l’appel distant.

Lors d'un `PUT` qui change les familles, le pays, la politique ou le format, remettre
...
#### Connecteurs sanitaires 2.5.0

Le module `source/payload/api/app/health_sources.py` est l'autorité pour le
registre, les identifiants de source et les parseurs purs. Les connecteurs actifs
et leurs points d'entrée sont :

- `who-gho` : `https://ghoapi.azureedge.net/api/Indicator`, OData, puis
  `/api/{IndicatorCode}` pour les observations ;
- `world-bank-health` :
  `https://api.worldbank.org/v2/source/2/indicator`, WDI, téléchargement
  `v2/en/indicator/{id}?downloadformat=csv` ;
- `unicef-sdmx` : catalogue `dataflow/all/all/latest` de l'API SDMX UNICEF ;
- `un-sdg` : `https://unstats.un.org/SDGAPI/v1/sdg/Indicator/List` ;
- `dhs` : `https://api.dhsprogram.com/rest/dhs/indicators` pour les indicateurs
  agrégés.

Ne transforme pas les 11 entrées `reference_portal` en connecteurs sans trouver
et vérifier une documentation API primaire stable. Les liens et contraintes de
compte doivent rester visibles dans l'onglet « Sources sanitaires ».
...
- une des sept sources dont `schedule_supported` vaut vrai ;
- requête ;
- limite de résultats ;
...
- afficher les quatre familles : COD-AB/COD-PS sélectionnables, COD-CS vide et COD-HP retiré désactivés ;
- afficher une liste de pays/zones intersection ONU M49 × HDX et ses compteurs ;
- peupler les sélecteurs de recherche et planification depuis `/api/sources` ;
- afficher 18 fiches de source en séparant `live_api` et `reference_portal` ;
- expliquer la portée statistique de M49 et la règle atomique des familles ;
...
- ajouter `cod_families` avec `["cod-ab"]` et `local_resources.cod_family` ;
- conserver un ancien profil déjà limité à un pays ou une zone M49 ;
- marquer tout ancien profil monde/région `migration_required`, désactiver son
  automatisation et exiger un choix dans la nouvelle liste commune.
...
source/
  build.sh
  HDP_Diagnostic_v2.5.0.cmd
  HDP_Configurer_GitHub_v2.5.0.cmd
  HumanitarianDataPlatform_Setup_README_v2.5.0.txt
  payload/
    CHANGELOG_HDP.log
    ...
    api/
      app/main.py
      app/health_sources.py
      app/project_integrations.py
      app/official_cod_cs_registry.json
  ...
  tests/test_v25_health_sources.py
```

### 20. Livrables v2.5.0 et empreintes

Dossier : `dist/v2.5.0`.

Fichiers principaux :

- `HumanitarianDataPlatform_Setup_Native_GUI_v2.5.0.exe` ;
- `HumanitarianDataPlatform_Setup_Native_GUI_v2.5.0.exe.sha256` ;
- `HumanitarianDataPlatform_Windows_v2.5.0.zip` ;
- `HumanitarianDataPlatform_Source_v2.5.0.zip` ;
- `HumanitarianDataPlatform_Archive_complete_v2.5.0.zip` ;
- `HumanitarianDataPlatform_Archive_complete_v2.5.0.zip.sha256` ;
- `HumanitarianDataPlatform_Setup_README_v2.5.0.txt` ;
- `CHANGELOG_HDP_v2.5.0.log` ;
- `HDP_Configurer_GitHub_v2.5.0.cmd` ;
- `HDP_Diagnostic_v2.5.0.cmd` ;
- `HDP_Prompt_exhaustif_reprise_GPT_Plus_v2.5.0.txt` ;
- `MANIFESTE_HumanitarianDataPlatform_v2.5.0.txt` ;
- `SHA256SUMS.txt`.

Les empreintes exactes doivent être lues dans `SHA256SUMS.txt`. L'archive
complète possède un fichier `.zip.sha256` séparé, car elle ne peut contenir sa
propre empreinte finale.
...
### 21. Validations réellement effectuées pour la v2.5.0

Les validations disponibles sont :

- compilation syntaxique des modules Python ;
- 36 tests unitaires réussis ;
- validation syntaxique JavaScript avec Node.js ;
- analyse de `compose.yaml` avec PyYAML ;
- génération du payload ;
- reconstruction de 19 fichiers du payload et comparaison à l’identique ;
- compilation de l’installateur ;
- contrôle PE32+ GUI x86‑64, 7 sections ;
- contrôle `unzip -t` des trois ZIP ;
- contrôle `sha256sum -c` ;
- contrôle des livrables historiques v2.3.2, v2.3.1, v2.3.0 et v2.0 ;
- parseurs des cinq nouveaux connecteurs validés sur des réponses représentatives
  sans dépendance réseau dans les tests ;
- aucune publication GitHub v2.5.0 effectuée dans cette remise.

Le moteur Docker était absent de l’environnement Linux de construction. Par conséquent, ne prétends pas que les éléments suivants ont été validés pour la v2.5.0 :
...
- appels réels aux cinq nouvelles API depuis HDP ;
...
- Respecte les quotas, licences et conditions d’utilisation de GitHub et de
  toutes les sources distantes.
- Les scripts stockés restent non exécutables.
- Le module n’accepte aucune saisie libre de jeu HDX : pays/zone M49, groupe ISO3 et famille COD officielle sont obligatoires.
- N'active jamais COD-CS sans entrée contrôlée dans son registre versionné et ne réintroduis pas COD-HP.
...
- OCHA Common Operational Datasets : <https://knowledge.base.unocha.org/wiki/spaces/imtoolbox/pages/42045911/Common%2BOperational%2BDatasets%2BCODs>
- OCHA COD-CS : <https://knowledge.base.unocha.org/wiki/spaces/imtoolbox/pages/2965897217/Country-specific%2BCODs%2BCOD-CS>
- CKAN Action API : <https://docs.ckan.org/en/latest/api/>
- ReliefWeb API V2 : <https://apidoc.reliefweb.int/>
- WHO GHO OData : <https://www.who.int/data/gho/info/gho-odata-api>
- World Bank Indicators API : <https://datahelpdesk.worldbank.org/knowledgebase/articles/889392-about-the-indicators-api-documentation>
- UNICEF SDMX : <https://data.unicef.org/sdmx-api-documentation/>
...
- les fonctions GitHub et le module M49/COD-AB/COD-PS officiel sont présents avec leurs confirmations et garde-fous ;
...
Ce prompt décrit l’état de livraison v2.5.0 ; les archives historiques 2.3.2 et antérieures restent inchangées.
