...
8. Appliquer obligatoirement `docs/prompts/audit_parametres_connecteurs_v7.md` : la qualification est paramètre-par-paramètre et comprend les tests de non-contamination UI/payload.
...
Pour chaque champ, auditer explicitement `UI → ID HTML → variable JS → JSON → modèle backend → concept HDP → paramètre natif`. Une modification d'un paramètre ne doit jamais contaminer un autre champ. Le cas `location=Rwanda` / `project_id` est un test permanent de non-régression.
...
## Phase F — qualification par fonctionnalité et par paramètre
Établir la liste des fonctionnalités et paramètres déclarés. Pour CHAQUE fonctionnalité critique et chaque paramètre critique, exécuter 10 cycles complets : test → observation → diagnostic → audit documentaire si anomalie → correction → nouveau test → audit de non-régression. Conserver le résultat de chaque cycle. Ne pas compter dix fois le même résultat sans réexécution réelle.

Statuts : PASS / FAIL / BLOCKED / NOT TESTED. Une fonctionnalité ne devient qualifiée que si les cycles requis passent ou si le protocole de release approuvé définit explicitement un autre seuil.

## Phase G — tests live
Après les tests déterministes, exécuter des sentinelles non destructives contre le fournisseur : cas nominal, géographie connue, période connue, résultat connu/non vide si possible, pagination, paramètres qualifiés et erreur contrôlée. Enregistrer HTTP status, URL/requête native expurgée, date, version de contrat et résultat. Une indisponibilité fournisseur est BLOCKED/PROVIDER_ERROR, jamais un faux zéro.
...
Créer dans `docs/versions/<version>/<connecteur>/` : audit API+nomenclatures, architecture, matrice fonctionnelle, matrice exhaustive des paramètres, protocole, résultats des cycles, qualification live, évaluation métier et rapport final. Maintenir un journal chronologique détaillé dans le dépôt avec commits, défauts, décisions et preuves.
...
## Critères de sortie
Un connecteur n'est `IMPLÉMENTÉ ET QUALIFIÉ` que si son implémentation de référence est unique, ses paramètres qualifiés sont exposés, chaque paramètre possède un statut et une preuve, les bindings UI/backend sont non contaminés, les nomenclatures ne sont pas devinées, les clients et la provenance sont cohérents, les tests déterministes requis passent, les tests live requis passent ou sont explicitement BLOCKED sans masquer le blocage, et la régression HDP complète est verte. La promotion release reste distincte de la qualification pour test utilisateur.
